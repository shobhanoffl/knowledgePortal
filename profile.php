...
if (isset($_GET['delType'])) {
  ...
  if($_GET['jno']){
    ...
    $update_col_query = "UPDATE users SET journals='$p_journals[0]' WHERE clgid='$p_clgid[0]'";
  ...
  if(mysqli_query($db, $del_query)){
      header('location: profile.php?msg=Post+Deleted&profile='.$p_clgid[0].'&getPosts=journal');
  }
  else{
      header('location: profile.php?msg=Post+Not+Deleted+-+Kindly+Raise+Issue&profile='.$p_clgid[0].'&getPosts=journal');
  }  
}
...
if (isset($_GET['chgProjStatus'])){
  ...
  if(mysqli_query($db,$proj_status_query)){
    ...
    header('location: home.php?Project+Status+Not+Updated+-+Kindly+Raise+Issue&getPosts=project');
  }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <link href="assets/app/css/profile-layout.css" rel="stylesheet">
  ...
  <div class="profile">
    ...
    <div class="profile-user-settings">
      ...
      <h1 class="profile-user-name">
        <span class="bg-effect">{{profileName}}</span>
      </h1>

      <p class="profile-dept-btn">
        <span class="bg-effect">{{profilePosition}}</span>
      </p>
    ...
    <div class="profile-bio">
      ...
      <p class="bg-effect">{{profileDept}}</p>
      <!-- This Line is Intentionally Commented Out -->
      <!-- <p class="bg-effect">{{profileBachelors}} {{profileMasters}} {{profileDoc}}</p> -->

    </div>

			<div class="profile-stats">

				<v-sheet class="vueSheet">
					<v-slide-group show-arrows center-active multiple>
						<ul>
						<v-slide-item>
							<li><span class="profile-stat-count">{{profilePostNos}}</span> <span class="profile-stat-caption">Journals</span></li>
						</v-slide-item>
						<v-slide-item>
							<li><span class="profile-stat-count">{{profileSeminarNos}}</span> <span class="profile-stat-caption">Seminars</span></li>
						</v-slide-item>
						<v-slide-item>
							<li><span class="profile-stat-count">{{profileWorkshopsNos}}</span> <span class="profile-stat-caption">Workshops</span></li>
						</v-slide-item>
						<v-slide-item>
							<li><span class="profile-stat-count">{{profileCourseNos}}</span> <span class="profile-stat-caption">Courses</span></li>
						</v-slide-item>
						<v-slide-item><li><span class="profile-stat-count">{{profileProjectsNos}}</span> <span class="profile-stat-caption">Projects</span></li></v-slide-item>
						<v-slide-item><li><span class="profile-stat-count">{{profileITNos}}</span> <span class="profile-stat-caption">Industrial Training</span></li></v-slide-item>
						<v-slide-item><li><span class="profile-stat-count">{{profileHackathonsNos}}</span> <span class="profile-stat-caption">Hackathons</span></li></v-slide-item>
						<v-slide-item><li><span class="profile-stat-count">{{profileFDPNos}}</span> <span class="profile-stat-caption">FDPs</span></li></v-slide-item>
						</ul>
					</v-slide-group>
				</v-sheet>
			</div>
...
<!-- PROFILE LAYOUT -->
<div class="post-wrap">
  <label for="typeOf">Filter by: </label>
  <select id="typeOf" name="typeOf" onchange="typeFilter()" selected="Select the Type of Data">
...
<!-- Caption -->
            ...
            <?php if(isset($rowSet[$i]['prno'])){ 
              ...
              <?php } ?>
                ...
                <?php if($rowSet[$i]['status']=="Completed"){ ?>
                  and Completed on 
                  <?php echo date("d-m-Y", $rowSet[$i]['enddt']).'.'; ?>
    ...
    <v-menu bottom left>
      ...
      <v-list>
        ...
        <v-list-item>
          <a href="<?php echo $current_url; ?>&chgProjStatus=<?php echo (isset($rowSet[$i]['prno'])) ? $rowSet[$i]['prno'] : NULL;  
...
<!-- POST TEMPLATE -->
  ...
  <script>
    new Vue({
      ...
      data:{
            // Profile Page
            profilePicURL:'<?php echo $profile_dp[0]; ?>',
            profileName:'<?php echo $profile_name[0]; ?>',
            profileDept:'Department of <?php echo $profile_dept[0]; ?>',
            profilePostNos:'<?php echo $profile_journals[0]; ?>',
            profileSeminarNos:'<?php echo $profile_seminars[0]; ?>',
            profileCourseNos:'<?php echo $profile_courses[0]; ?>',
            profilePosition:'<?php echo $profile_design[0]; ?>',
			      profileWorkshopsNos:'<?php echo $profile_wshops[0]; ?>',
			      profileProjectsNos:'<?php echo $profile_proj[0]; ?>',
			      profileITNos:'<?php echo $profile_it[0]; ?>',
			      profileHackathonsNos:'<?php echo $profile_hackathons[0]; ?>',
			      profileFDPNos:'<?php echo $profile_fdp[0]; ?>',
            // Snackbar
            <?php if (isset($_GET['msg'])){ ?>
              snackbar: true,
              timeout: 4000,
    ...
    // Update Query to Display in Profile
    ...
    function typeFilter(){
      ...
      var getPosts = getQueryVariable(url, 'getPosts');
      var typeVal = document.getElementById("typeOf").value;
      var new_url = url.replace(getPosts,typeVal);
